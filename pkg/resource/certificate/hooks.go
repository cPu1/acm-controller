...
func (rm *resourceManager) maybeImportCertificate(ctx context.Context, r *resource) (*resource, bool, error) {
	...
	if certSpec.Certificate != nil {
		if certSpec.DomainName != nil || certSpec.DomainValidationOptions != nil || certSpec.KeyAlgorithm != nil ||
			certSpec.SubjectAlternativeNames != nil || certSpec.Options != nil {
			return nil, false, ackerr.NewTerminalError(errors.New("cannot set fields used for requesting a new certificate when importing a certificate"))
		}
		...
		if len(input.PrivateKey) == 0 {
			return nil, false, errors.New("privateKey is required when importing a certificate")
		}
	...
	if certSpec.DomainName != nil {
		if certSpec.Certificate != nil || certSpec.PrivateKey != nil || len(certSpec.CertificateChain) > 0 {
			return nil, false, ackerr.NewTerminalError(errors.New("cannot set fields used for importing a new certificate when requesting a new certificate"))
		}
	}
...
// code-generator does not resolve secret key references for custom fields like PrivateKey and Certificate.
